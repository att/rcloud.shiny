# we override a few functions in shiny by copying, renaming, and slighly modifying them.
# The purpose of these changes are to display cells as shiny app sources in showcase mode.
...
# Source copied from https://github.com/rstudio/shiny/blob/df30a3c7f41f33c2f9918c034727e4b332a06bcf/R/showcase.R
  ...
  html <- with(tags, tagList(
    ...
    if (file.exists(mdfile))
      script(type="text/markdown", id="showcase-markdown-content",
             paste(shiny:::readUTF8(mdfile), collapse="\n"))
    else ""
  ))
